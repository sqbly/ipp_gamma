...
/** Struktura przechowująca parametry gry.
 */
typedef struct {
...
static int many_games(void) {
    ...
    for (size_t i = 0; i < MANY_GAMES; ++i) {
        for (size_t j = 0; j < SIZE(game); ++j) {
            ...
            assert(
                gamma_move(g[i][j], 1, game[j].width - 1, game[j].height - 1));
...
static int big_board(void) {
    static const gamma_param_t game[] = {
        ...
        {MIDDLE_BOARD_SIZE, MIDDLE_BOARD_SIZE, MIDDLE_BOARD_SIZE,
         MIDDLE_BOARD_SIZE},
        {SMALL_BOARD_SIZE, SMALL_BOARD_SIZE, SMALL_BOARD_SIZE,
         SMALL_BOARD_SIZE},
    };
...
static int middle_board(void) {
    ...
    for (uint32_t size = 8000; size >= 125; size /= 2) {
        ...
        if (g1 != NULL) {
            gamma_delete(g1);
            // Jeśli udało się zaalokować planszę o rozmiarze size * size, to
            // powinno się też udać zaalokować inną planszę o nieco mniejszej
            // powierzchni.
            gamma_t *g2;
...
/** Struktura przechowująca test.
 */
typedef struct {
    char const *name;       ///< nazwa testu
    int (*function)(void);  ///< wskaźnik na funkcję przechowującą test
} test_list_t;
